...
The state of the art for natural language processing has achieved important progress in answering questions algorithms; like: \cite{agichtein:2004}, \cite{start:2015} and \cite{bendersky:2008}% Enumerate Agichtein, E., Lawrence, S., and Gravano, L. 2004. Learning to find answers to questions on the web.
...
The information extracted from the questions will be used to generate a n-dimensional description vector for the question under analysis. The description vector is going to be compared using cosine distance against another description vector previously analyzed. If the resulting distance is less than a given threshold \emph{t} then both questions are going to be considered equivalent. The answer of the previously analyzed question will be set as candidate answer of the new question. The candidate answer will be presented to the expert human for evaluation. As the questions and answers database grows the searches will take more time each time. To overcome that situation, the questions will be classified based on a hashing function like minhashing. The algorithm is the following: 
\begin{enumerate}
...
In natural language different terms may be used to refer to the same concept. In such a case the two concepts are considered synonyms if for both of them there is a path to a fixed annotation which is the same for both with respect to a common context%confirm if this can be done.
